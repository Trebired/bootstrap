...
async function writeConsumerPackageJson(consumerDir, tarballPath) {
  await fs.writeFile(path.join(consumerDir, "package.json"), JSON.stringify({
        name: "bootstrap-pack-smoke",
        private: true,
        type: "module",
        dependencies: {
          "@package/bootstrap": `file:${tarballPath}`,
...
async function writeConsumerSourceFiles(consumerDir) {
  ...
  await fs.writeFile(path.join(consumerDir, "index.ts"), [
      ...
      "console.log(",
      "  Boolean(runtime),",
      "  Boolean(lifecycleLogger),",
      "  Boolean(shutdownController),",
      "  Boolean(signalBinder),",
      "  Boolean(entrypoint),",
      ");",
    ].join("\n"));
  ...
  await fs.writeFile(path.join(consumerDir, "runtime.mjs"), [
      ...
      "console.log(",
      "  typeof bootstrap,",
      "  typeof createBootstrap,",
      "  typeof createBootstrapLifecycleLogger,",
      "  typeof createBootstrapShutdownController,",
      "  typeof bindBootstrapShutdownSignals,",
      ");",
    ].join("\n"));
}
